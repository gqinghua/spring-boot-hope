...
import com.data.hope.curd.dto.CodeColumn;
 ...
 * @since 2022-03-01
 */
public abstract class ReportBaseController<P extends PageParam, T extends BaseEntity, D extends BaseDTO> extends ReportController {
    ...
    @Transactional
    public ResponseBean insert(@Validated @RequestBody D dto) throws Exception {
    ...
    @Transactional
    public ResponseBean deleteBatchIds(@RequestBody List<String> ids) {
     ...
     * @return
     */
    @PostMapping("/queryByDatas")
    @ReportAuditLog(pageTitle = "指定字段数据查询服务")
    @ApiOperation(value = "指定字段数据查询服务:根据指定的字段查询数据 维护人：郭清华")
    @Transactional
    public ResponseBean queryByColumnDatas(@RequestBody CodeColumn codeColumn) {
        logger.info("{}指定字段数据查询服务，参数：{}", this.getClass().getSimpleName(), ReportUtils.toJSONString(codeColumn));

        T entity = getEntity();
        //dto转为数据库实体

        List<T> ts = getService().selectList(codeColumn.getColumn(), codeColumn.getSetName());

        ResponseBean responseBean = responseSuccessWithData(ts);

        logger.info("{}指定字段数据查询服务，结果：{}", this.getClass().getSimpleName(), ReportUtils.toJSONString(responseBean));
        return responseBean;
    }
    ...
    @ReportAuditLog(pageTitle = "查询全部")
    @ApiOperation(value = "查询全部 维护人：郭清华")
    public ResponseBean queryByList(P param) {
        IPage<T> iPage = getService().page(param);
        List<T> records = iPage.getRecords();
    ...
    @ReportAuditLog(pageTitle = "LIST查询")
    @ApiOperation(value = "LIST查询 维护人：郭清华")
    public ResponseBean queryList() {
        List<T> all = getService().findAll();
        ResponseBean responseBean = responseSuccessWithData(all);
